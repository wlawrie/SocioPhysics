...
class cluster():
    ...
    def __init__(self,init_x,init_y,val,grid_x,grid_y,grid,attitudes,threshold):
        ...
        self.attitude_binary   = np.zeros((grid_x,grid_y))
    ...
    def perim_check(self,x,y): #Perimiter check for the grid
         perim = 0
         nn = [[0,1],[0,-1],[1,0],[-1,0]]        
         for i in range(4):
    ...
    def construct_attitude_binary(self):
        self.attitude_binary = np.where(np.sum(self.attitudes/4.0,axis=2)>self.threshold, 1,0)
...
init = 0.9
n_avs = 1
adjust = np.linspace(0.0101, 0.010101, num=n_avs)
...
for i in range(0,n_avs):
    ...
    #while t < timesteps:
        ...
        if(np.mod(100*t/timesteps,1)==0):
            ...
            the_grid.plot_grid()
            the_grid.plot_attitudes()
            print(np.sum(the_grid.DOIAttitudes[t]),np.sum(the_grid.attitudes)/(4.0*length*width))
...
for i in range(length):
    for j in range(width):
        if (not [i,j] in closed_queue):
            a_cluster = cluster(i,j,the_grid.opinions[i,j],length,width,the_grid.opinions,the_grid.attitudes,att_threshold)
...
a_cluster.construct_attitude_binary()   
closed_queue = []
cluster_counter = 0
cluster_areas = []
cluster_perimiters = []
...
for i in range(length):
    for j in range(width):
        if (not [i,j] in closed_queue):
            a_nother_cluster = cluster(i,j,the_grid.opinions[i,j],length,width,a_cluster.attitude_binary,the_grid.attitudes,att_threshold)
            a_nother_cluster.create_coord_list()
            a_nother_cluster.calculate_area()
            a_nother_cluster.calculate_perimeter()
            
            closed_queue+=a_nother_cluster.coords
            cluster_areas.append( a_nother_cluster.area)
            cluster_perimiters.append(a_nother_cluster.perimiter)
            cluster_counter += 1
